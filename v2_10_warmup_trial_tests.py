...
def run():
    checks = 0
    assert APP_VERSION == "2.21.0"; checks += 1
    ...
    assert APP.index("weekly_quiz_state = render_friday_quiz(store, day)") < APP.index('if weekly_quiz_state != "complete" and not render_quick_warmup(store, day):') < APP.index("day, facts, challenge, attempt = load_student_daily_context(store)"); checks += 1
