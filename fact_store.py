...
class InMemoryFactStore:
    ...
    def mark_alternate_fix_complete(
        ...
        if progress.fix_completed_at is not None:
            return progress
        updated = replace(progress, fix_completed_at=utc_now())
    ...
    def set_alternate_focus_plan(
        ...
        if progress.focus_plan:
            return progress
        items = tuple(dict(item) for item in plan)
        ...
        updated = replace(progress, focus_plan=items)
        self.alternate_learning_progress[(str(student_id), str(challenge_id))] = updated
        return updated
    ...
    def recent_alternate_learning_events(
        ...
        rows = [row for row in self.alternate_learning_events if row.student_id == str(student_id)]
    ...
    def record_alternate_focus_batch(
        ...
        for index, batch in submitted_by_index.items():
            ...
            for position, raw in enumerate(batch):
                ...
                self._append_alternate_learning_event(
                    student_id=attempt.student_id, challenge_id=attempt.challenge_id, attempt_id=attempt.attempt_id,
                    daily_mode=attempt.daily_mode, activity_type="focus", activity_index=index,
                    domain=identity.domain, skill_key=identity.skill_key, skill_label=identity.skill_label,
                    item_key=identity.item_key, prompt=str(item.get("prompt") or ""), student_answer=value,
                    correct_answer=expected, correct=value == expected, is_retry=is_retry,
                    client_event_id=str(raw.get("client_event_id")), response_seconds=seconds,
                )
