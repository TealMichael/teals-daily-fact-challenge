...
-- v2.21.0 Quiz of the Week foundation (no student names / Skyward identifiers)
create table if not exists public.weekly_quiz_sets (
    ...
    constraint weekly_quiz_exactly_five_questions check (jsonb_typeof(questions) = 'array' and jsonb_array_length(questions) = 5)
);
...
create table if not exists public.weekly_quiz_answers (
    ...
    constraint weekly_quiz_type_allowed check (question_type in ('Number','Fraction','Multiple choice','Number + Label')),
    constraint weekly_quiz_prompt_not_blank check (length(btrim(prompt)) between 1 and 2000)
);
create index if not exists weekly_quiz_sets_class_date_idx on public.weekly_quiz_sets(class_id, quiz_date);
create index if not exists weekly_quiz_answers_quiz_student_idx on public.weekly_quiz_answers(quiz_id, student_id, question_slot);
create index if not exists weekly_quiz_answers_date_class_idx on public.weekly_quiz_answers(quiz_date, class_id);
