...
for mode in alt_modes:
    ...
    check(f"{mode} perfect auto-completes Fix", progress is not None and progress.completed_at is not None)
...
for mode in alt_modes:
    ...
    check(f"{mode} corrected Fix completes routine", progress is not None and progress.completed_at is not None)
...
check("alternate completed Fix qualifies Mystery repair", store.completed_mystery_days(student.student_id, date(2026, 8, 31), through_day_number=1) == [(1, challenge.challenge_id)])

# Teacher Today is mode-aware: alternate completion can be in Fix, never Focus.
rows = [
...
check("teacher Mixed completed follow-up shows Done", summary["done"] == 1 and summary["focus"] == 0)
